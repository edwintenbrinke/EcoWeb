...
$(function () {
  ...
  if (lang == null) { lang = navigator.language.substring(0, 2) } else { $('#langSelect').val(lang) }
  setTimeout(function () {
...
function translate (tObject, tClass) {
  $(tClass).each(function (index) {
    const translateKey = $(this).attr('translate-key')
    const translation = tObject[translateKey]
    ...
    } else if (!$(this).attr('placeholder') && !$(this).html()) {
