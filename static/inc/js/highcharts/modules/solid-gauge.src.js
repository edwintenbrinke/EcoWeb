...
(function (H) {
  ...
  H.seriesTypes.solidgauge = H.extendClass(H.seriesTypes.gauge, {
    ...
    drawPoints () {
      ...
      const overshoot = options.overshoot
      const overshootVal = overshoot && typeof overshoot === 'number' ? overshoot / 180 * Math.PI : 0

      H.each(series.points, function (point) {
        const graphic = point.graphic
        let rotation = yAxis.startAngleRad + yAxis.translate(point.y, null, null, null, true)
        const radius = (pInt(pick(point.options.radius, options.radius, 100)) * center[2]) / 200
        const innerRadius = (pInt(pick(point.options.innerRadius, options.innerRadius, 60)) * center[2]) / 200
        let shapeArgs
        let d
        let toColor = yAxis.toColor(point.y, point)
        let fromColor
        const axisMinAngle = Math.min(yAxis.startAngleRad, yAxis.endAngleRad)
        const axisMaxAngle = Math.max(yAxis.startAngleRad, yAxis.endAngleRad)
        let minAngle
        let maxAngle
