...
(function (H) {
  ...
  Chart.prototype.applyDrilldown = function () {
    const drilldownLevels = this.drilldownLevels
    let levelToRemove
  ...
  Chart.prototype.drillUp = function () {
    ...
    const levelNumber = drilldownLevels[drilldownLevels.length - 1].levelNumber
    ...
    let newSeries
    let oldExtremes
    const addSeries = function (seriesOptions) {
      let addedSeries
      ...
      setTimeout(function () {
        if (newSeries.points) { // May be destroyed in the meantime, #3389
          each(newSeries.points, function (point, i) {
            ...
            const inherit = verb === 'show' ? true : undefined
            if (point.graphic) { // #3407
  ...
  ColumnSeries.prototype.animateDrilldown = function (init) {
    const series = this
    const drilldownLevels = this.chart.drilldownLevels
    let animateFrom
      ...
      if (graphic) {
        ...
        if (animationOptions) {
          graphic.animate(
            ...
            H.merge(animationOptions, { complete })
          )
        } else {
  ...
  H.Axis.prototype.drilldownCategory = function (x) {
    let key
    let point
    const ddPointsX = this.ddPoints[x]
  ...
  Tick.prototype.drillable = function () {
    ...
    const ddPointsX = axis.ddPoints && axis.ddPoints[pos]
  ...
  wrap(H.Point.prototype, 'init', function (proceed, series, options, x) {
    ...
    const tick = xAxis && xAxis.ticks[x]
    const ddPointsX = xAxis && xAxis.getDDPoints(x, series.options._levelNumber)
