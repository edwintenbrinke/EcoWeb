    ...
    svg.Font = new function () {
      ...
      this.Parse = function (s) {
        ...
        for (let i = 0; i < d.length; i++) {
          if (!set.fontStyle && that.Styles.includes(d[i])) { if (d[i] != 'inherit') { f.fontStyle = d[i] } set.fontStyle = true } else if (!set.fontVariant && that.Variants.includes(d[i])) { if (d[i] != 'inherit') { f.fontVariant = d[i] } set.fontStyle = set.fontVariant = true	} else if (!set.fontWeight && that.Weights.includes(d[i])) {	if (d[i] != 'inherit') { f.fontWeight = d[i] } set.fontStyle = set.fontVariant = set.fontWeight = true } else if (!set.fontSize) { if (d[i] != 'inherit') { f.fontSize = d[i].split('/')[0] } set.fontStyle = set.fontVariant = set.fontWeight = set.fontSize = true } else if (d[i] != 'inherit') { ff += d[i] }
    ...
    svg.Element.text = function (node) {
      ...
      this.renderChildren = function (ctx) {
        ...
        for (let i = 0; i < this.children.length; i++) {
          ...
          const childLength = child.measureText ? child.measureText(ctx) : 0
...
(function (Highcharts) { // encapsulate
  ...
  extend(CanVGRenderer.prototype, {
    ...
    setContainer (container, chartWidth, chartHeight) {
