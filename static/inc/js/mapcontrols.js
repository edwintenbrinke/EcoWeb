const MapControls = function () {
  ...
  this.bindEvents = function () {
    ...
    $('.map-layer-select').on('change', function () {
      const mapView = this.parentElement.nextElementSibling
    ...
    $('#map-toggle-play').click(function () {
      const newState = !mainMap.settings.pause
    ...
    $(document).on('click', '.fancy-add-map', function () {
      ...
      const view = document.getElementById('main-map')
    ...
    $(document).on('click', '.fancy-add-district', function () {
      ...
      let mapdrav = localStorage.getItem('mapdrav')
      ...
      const districts = $('.district-item')
      for (let i = 0; i < districts.length; i++) {
        const district = districts[i]
        const id = districts[i].getAttribute('data-id')
        const color = $(district).css('background-color')
...
$(document).ready(function () {
  const mapcontrol = new MapControls()
...
function SwitchButtons (buttonId) {
  let hideBtn, showBtn, menuToggle
  if (buttonId == 'button1') {
    menuToggle = 'menu2'
    showBtn = 'button2'
    hideBtn = 'button1'
...
$(document).ready(function () {
  if (currentPage == 'addlaw') {
    const cp = ColorPicker(document.getElementById('flexi-color-slide'), document.getElementById('flexi-color-picker'),
      function (hex, hsv, rgb, mousePicker, mouseSlide) {
        ...
        const districtId = $('.district-item.active')[0].getAttribute('data-id')
        ...
        $('.district-item').each(function () {
          ...
          const val = $(this).find('.district-name-input').val()
          item.bg = bg
          item.val = val
          item.id = id
          mapinfo[id] = item
  ...
  $(document).on('click', '.district-item', function (event) {
    ...
    if (currentPage == 'addlaw') {
      ...
      if ($(this).hasClass('active')) {
        ...
        const color = $(this).css('background-color').split(')')[0].split('(')[1].split(', ')
        const rgb = {
          r: color[0],
          g: color[1],
          b: color[2]
  ...
  $(document).on('click', '.remove_district_item', function () {
    ...
    const arr = toByteArray(ECO.MINIMAP.GetZone())
    for (var i = 0; i < arr.length; i++) {
      if (arr[i] == id) {
        arr[i] = 0
      }
    }
    const newZone = fromByteArray(arr)
    ...
    if ($('.district-item').length == 1) { $('.remove_district_item').remove() }
  })
  ...
  $(document).on('click', '.district-item .district-name-input, .district-item .district-button', function (event) {
    event.stopPropagation()
    const districtId = this.parentNode.getAttribute('data-id')
    ECO.MINIMAP.paintDistrict(districtId)
    const mapinfo = {}
    $('.district-item').each(function () {
      const item = {}
