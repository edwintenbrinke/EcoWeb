;(function () {
  var Law = function (id) {
    ...
    this.getAllLaw = function (callback, update) {
      update = update || false
      const url = APPIQUERYURL + '/api/v1/laws?authtoken=' + law.authtoken
      const _this = this
    ...
    this.getAllComments = function (callback, update) {
      const url = APPIQUERYURL + '/api/v1/laws/getcomments?lawguid=' + law.currentLawID + '&authtoken=' + law.authtoken
      const _this = this
    ...
    this.removeComment = function (comment) {
      ...
      const data = { Username: law.username, Text: comment }

      const url = `${APPIQUERYURL}/api/v1/laws/removecomment?authtoken=${law.authtoken}&lawguid=${law.currentLawID}`
    ...
    this.GetComments = function (type) {
      ...
      function getComments (data) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let comment = data[i].Text
          // hack for fixing old format comments
          if (comment[0] === '"') { comment = comment.substring(1, comment.length - 1).replace(/<br>/g, '\n') }
          // getting days, hours and minutes from timestamp value of query
          const timestamp = data[i].Timestamp
          const commentTime = "<span class='localize' translate-key='146'>Posted on</span>  " + ConvertSecondsComments(timestamp)
          ...
          const section = $("<div class='comment-section1'> </div>")
          ...
          if (data[i].Map != null && data[i].Map != '') {
            ...
            const viewer = $('<div class="view-map" id=map-' + i + '"></div>')
    ...
    this.addComment = function (comment, isEdited) {
      ...
      if (($('#comment').val() != null && $('#comment').val() != '') || comment != null) {
        const url = APPIQUERYURL + '/api/v1/laws/addcomment?authtoken=' + law.authtoken + '&lawguid=' + law.currentLawID
    ...
    this.sendVote = function (vote) {
      ...
      const currentLawID = law.currentLawID

      const url = APPIQUERYURL + '/api/v1/laws/' + currentLawID + '/vote/' + ((vote == 0) ? 'true' : 'false') + '?authtoken=' + law.authtoken
  ...
  function bindEvents () {
    ...
    const ignore = (getURLParameter('ignoretownhall') == 'true' ? '&ignoretownhall=true' : '')
    ...
    $(document).on('click', '#removeComment', function () {
      const content = $(this).parent().find('.law-discussion-comment').html()
    ...
    $(document).on('click', '#editComment', function () {
      ...
      const graphs = $(this).attr('data-graph')
