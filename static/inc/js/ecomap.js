(function (global, factory) {
	    ...
	    const parseExt = function (block) {
	        ...
	        const parseComExt = function (block) {
	            if (st.peek() == 4) {
	                ...
	                st.readByte()
	            } else { block.comment = parseFloat(readSubBlocks()) }
  ...
  function GIFDECODER (name, OnFinish, filter) {
	    ...
	    my.clean = function (name) {
	        ...
	        for (let i = 0; i < GIFMANAGER.gifStorage[name].length; i++) {
	    ...
	    my.loadImage = function (name) {
	        ...
	        xhr.onload = function (e) {
	            const arrayBuffer = xhr.responseText // Note: not xhr.responseText
	    ...
	    const doImg = function (img) {
	        ...
	        const ct = img.lctFlag ? img.lct : hdr.gct

	        const cData = frame.getImageData(img.leftPos, img.topPos, img.width, img.height)
	    ...
	    const doComment = function (cblock) {
  ...
  var GIFMANAGER = (function () {
	    ...
	    my.giftextures = function (name, onFinish, filter = THREE.LinearFilter) {
	        if (!my.gifStorage[name]) {
	            ...
	            my.gifStorage[name] = []
	            my.gifTimes[name] = []
	            const tempLoader = new GIFDECODER(name, onFinish || my.OnGifLoaded, filter)
  ...
  function GenChunk (xSize, zSize, worldX, worldZ) {
	    ...
	    const v = new THREE.BufferAttribute(new Float32Array((xSize + 1) * (zSize + 1) * 4 * 4), 4)
	    const t = new THREE.BufferAttribute(new Uint16Array(xSize * zSize * 18), 1)
	    ...
	    const d = Math.round(worldX / xSize)
	    const instances = d * d
	    const o = new THREE.InstancedBufferAttribute(new Float32Array(instances * 3), 3, 1)
	    i = 0
	    for (var z = 0; z < d; z++) {
	        for (var x = 0; x < d; x++) {
  ...
  PlotMap.prototype.DrawLine = function (start, end, size = 1) {
	    if (start.distanceTo(end) > this.image.width / 2) {
	        ...
	        const dx = Math.abs(end.x - vec.x)
	        const dy = Math.abs(end.y - vec.y)
	        const sx = (vec.x < end.x) ? 1 : -1
	        const sy = (vec.y < end.y) ? 1 : -1
	        let err = dx - dy
  ...
  PlotMap.prototype.SetRect = function (a, b) {
	    ...
	    const min = new THREE.Vector2((a.x < b.x) ? a.x : b.x, (a.y < b.y) ? a.y : b.y)
	    const size = new THREE.Vector2(Math.abs(a.x - b.x), Math.abs(a.y - b.y))
  ...
  PlotMap.prototype.AntiAntiAlias = function () {
	    const data = this.ctxt.getImageData(0, 0, this.image.width, this.image.height)
  ...
  PlotMap.prototype.ApplyDistrictColors = function (colorsArray) {
	    ...
	    for (let i = 0; i < data.data.length; i += 4) {
	        const districtOffset = data.data[i] * 4
	        for (let j = 0; j < 4; j++) { data.data[i + j] = colorsArray[districtOffset + j] }
	    }
	    this.ctxt.putImageData(data, 0, 0)
	    this.ctxt.needsUpdate = true
  }
  ...
  PlotMap.prototype.LoadDistrictMap = function (districtData) {
	    ...
	    const colorsArray = new Uint8Array(256 * 4)
	    colorsArray[0] = 255 // 0 is white / "no district"
	    colorsArray[1] = 255
	    colorsArray[2] = 255
	    colorsArray[3] = 255
	    for (let i = 0; i < districtData.DistrictMetadata.length; i++) {
	        const id = districtData.DistrictMetadata[i].ID
	    ...
	    if (districtData.DistrictMap != undefined) { this.LoadArray(districtData.DistrictMap) }
  ...
  DistrictColors.prototype.SetPixel = function (vec, color) {
	    const id = this.ctxt.createImageData(1, 1)
	    id.data[0] = color.r
	    id.data[1] = color.g
	    id.data[2] = color.b
	    id.data[3] = color.a
  ...
  const MINIMAP = (function () {
	    ...
	    let scenes = []; let views = []; let t; let canvas; let renderer
	    const heatmap = new THREE.TextureLoader().load('inc/images/heatmapinvert.png')
	    let featuremap, drawmap, districtMap, proposedDistrictMap, districtColorLookup
	    const terrainmap = new THREE.TextureLoader().load('Layers/TerrainLatest.gif')
	    ...
	    let worldWidth = 200; let worldDepth = 200
	        let worldHalfWidth = worldWidth / 2; let worldHalfDepth = worldDepth / 2

	    let heightMap; let heightHistoryLoaded = false; let terrainHistoryLoaded = false

	    my.init = function () {
	        const texture = new THREE.TextureLoader().load('Layers/HeightMapLatest.gif', function (texture) {
	            ...
	            if (views.length && my.waterLevel > 0) { finishInit(texture) }
	        })
	    ...
	    const flatShaderDefines = { GRID: true, FLAT: true, WRAP: true, FMAP: true }
	    const curveShaderDefines = { GRID: true, FMAP: true }
	    const waterFlatShaderDefines = { FLAT: true, WRAP: true }
	    const waterCurveShaderDefines = {}
	    ...
	    my.addView = function (view) {
	        ...
	        if (!my.initialized) { finishInit(heightMap) }
	        ...
	        if (my.zoneString) { view.mainMat.defines.DRAWING = true }
	        ...
	        if (view.settings.noWater !== undefined && view.settings.noWater) { view.mainMat.uniforms.waterColor.value = new THREE.Color(0.0, 0.0, 0.1) }

	        my.setLayer(view.settings.layerSelected, view)

	        const renderLayer = new THREE.ShaderPass(layerShader)
	        renderLayer.renderToScreen = true

	        const mesh = new THREE.Mesh(geometry, view.positionLookupShader) // material);
	        ...
	        const plane = new THREE.Mesh(geometry2, waterMat)
	        ...
	        if (view.camera) { scene.userData.camera = view.camera } else {
	    ...
	    function initPostprocessing (scene, camera, rect) {
	        ...
	        const pars = { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBAFormat, type: THREE.FloatType }
	    ...
	    my.removeView = function (view) {
	        if (scenes.length == 1 && scenes[0].userData.view == view) { scenes = [] } else {
	    ...
	    function generateHeight (hmap) {
	        ...
	        const size = hmap.image.width * hmap.image.height; let data = new Float32Array(size)

	        context.drawImage(hmap.image, 0, 0)
	        const imgd = context.getImageData(0, 0, hmap.image.width, hmap.image.height)
	        const pix = imgd.data
	    ...
	    const getWorldPos = function (position) {
	    ...
	    function GetFrameNum (name, frameNum, view) {
	        if (GIFMANAGER.gifTimes[name] != undefined && GIFMANAGER.gifTimes[name].length > 0) {
	            ...
	            const playSpeed = view.settings.playSpeed
	    ...
	    let pos; let mouseHitWorld = false
	    function render () {
	        updateSize()
	        ...
	        scenes.forEach(function (scene) {
	            const view = scene.userData.view
	            ...
	            if (!s.pause && GIFMANAGER.gifStorage.HeightMap === undefined) {
	                GIFMANAGER.giftextures('HeightMap', function (x) {
            heightHistoryLoaded = true
	            ...
	            const left = rect.left - myRect.left
	            const top = rect.top
	            ...
	            let materialChanged = false; let heightChanged = false
	            const lastTime = s.currentTime; const prevFrameNum = s.frameNum; const prevHeightFrameNum = s.heightFrameNum; const prevTerrainFrameNum = s.terrainFrameNum
	            ...
	            if (lastTime != s.currentTime || view.forceUpdate) {
	                if (view.timeText) { view.timeText.textContent = s.currentTime / 3600.0 }
	                if (view.OnTimeUpdated) { view.OnTimeUpdated(s.currentTime) }

	                s.frameNum = GetFrameNum(s.layerSelected, s.frameNum, view)
	                if (heightHistoryLoaded) { s.heightFrameNum = GetFrameNum('HeightMap', s.heightFrameNum, view) }
	                if (terrainHistoryLoaded) { s.terrainFrameNum = GetFrameNum('Terrain', s.terrainFrameNum, view) }
	                ...
	                if (materialChanged) { view.mainMat.needsUpdate = true }
	            }

	            const mouseViewX = mouseX - rect.left
	            const mouseViewY = rect.height - (mouseY - rect.top)
	            ...
	            if (view.OnPositionUpdated) {
	                if (pos != view.worldPos) {
	                    ...
	                    if (view.OnInfoUpdated) {
	                        const lookup = getPlotNum(pos.x, pos.y)
	                        let plotName = '\n'
	                        if (plotLookup[lookup] !== undefined) { plotName = plotNameLookup[plotLookup[lookup]] }
	            ...
	            if (cam.position.x > worldHalfWidth * 100) { cam.position.x -= worldWidth * 100 } else if (cam.position.x < -worldHalfWidth * 100) { cam.position.x += worldWidth * 100 }
	            if (cam.position.z > worldHalfDepth * 100) { cam.position.z -= worldDepth * 100 } else if (cam.position.z < -worldHalfDepth * 100) { cam.position.z += worldDepth * 100 }
	            ...
	            if (view.cameraText) {
	                view.cameraText.textContent = JSON.stringify(s)
	            }
	    ...
	    const urlGameServer = parent.APPIQUERYURL
	    my.layerNames = ['Camas', 'Elk']
	    const layerSelected = 'Fireweed'
	    my.waterLevel = -1
	    ...
	    function setupLayers () {
	        $.ajax({
	            ...
	            success (data, status, xhr) {
	                ...
	                plots = data.Plots
	                if (heightMap !== undefined) { finishInit(heightMap) }
	            }
	        })
	    }
	    let plots
	    ...
	    my.GetZone = function () {
	        if (my.drawZone) { return drawmap.GetArray() } else { return '' }
	    }

	    my.SetZone = function (b64string) {
	        if (this.initialized) { drawmap.LoadArray(b64string) } else { my.zoneString = b64string }
	    }

	    my.LoadProposedDistricts = function (districtData) {
	        if (proposedDistrictMap === undefined) { proposedDistrictMap = new PlotMap(worldWidth / 5, worldDepth / 5) }
	        if (proposedDistrictMap.metadata !== undefined) { return }
	        proposedDistrictMap.LoadDistrictMap(districtData)
	        const select = $('.map-layer-select')
	        const option = $('<option></option>').attr('value', 'ProposedDistricts').text('ProposedDistricts')
	    ...
	    function setupPlots () {
	        ...
	        let i = 0
	        for (const prop in plots) {
	            plotNameLookup[i] = prop
	            for (const p in plots[prop]) {
	                const v = plots[prop][p]
	    ...
	    my.setLayer = function (layerName, viewRef = undefined) {
	        const view = viewRef || scenes[scenes.length - 1].userData.view
	    ...
	    function screenToWorldPos (view, viewX, viewY) {
	        ...
	        const hitWorld = read[3] >= 0.7 // AO is 0.7-1.0
	        if (hitWorld) { return new THREE.Vector2(Math.floor(read[0] * worldWidth), worldDepth - Math.floor(read[1] * worldDepth)) }
	        return null
	    }
