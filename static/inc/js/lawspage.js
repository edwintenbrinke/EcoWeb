let LASTINTERVALID = null // global variable which will help counter to work right
...
function lawspage (lawID, curentLaw, Law, action = 'load') {
  ...
  function law_load (lawsData) {
    ...
    const this_law = array_filter(lawsData, filter_function)
    ...
    const currentLawID = checkedData.Id
    ...
    let ratifedDay
    let ratifedHour
    const ratifedEnding = " <span class='localize' translate-key='154'>ago</span>"
    // if (days == 1) {
    ...
    const description = checkedData.Description
    ...
    let lawText = checkedData.Description.replace(/\n/g, '<br />')
    ...
    if (checkedData.Map != null && checkedData.Map != '') {
      ...
      if (checkedData.Description.includes('districtEditor') && $('.district-controls-wrapper').length == 0) {
        const fieldId = $('#law-text').find('districteditor').attr('fieldid')
      ...
      if ($('#law-reasoning-description-map-viewer').length == 0) {
        const viewer = $('<div class="view-map" id="law-reasoning-description-map-viewer"><p id="map-position-text"></p><p id="map-info-text">.</p><div class="district-controls-wrapper"><div class="district-title toggle_prposed_district">Proposed Districts</div><input type="hidden" class="district-wrap"></div></div>')
        ...
        view.OnPositionUpdated = function (pos) {
          view.children[0].textContent = pos.x + ', ' + pos.y
        }
        view.OnInfoUpdated = function (text) {
          view.children[1].textContent = text
        }

        // Load District
        if (checkedData.District != null && checkedData.District != '') { ECO.MINIMAP.SetZone(checkedData.District) }
      }
    ...
    const isActive = $('#active-laws a#' + getURLParameter('lawid')).length
...
function counter (t) {
  ...
  let seconds = parseInt((((t - hours * 3600) - minutes * 60)))

  function counterDisplay () {
    let displayhours = ' ' + hours + ' '
    let displayminutes = ' ' + minutes + ' '
    const displayseconds = ' ' + ('00' + seconds).slice(-2) + ' ' // add leading 0 when under 10 -> 09, 08, 07..
