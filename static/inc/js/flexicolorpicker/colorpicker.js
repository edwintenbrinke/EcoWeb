...
(function (window, document, undefined) {
  ...
  let picker; let slide; const hueOffset = 15; const svgNS = 'http://www.w3.org/2000/svg'
  ...
  function $ (el, attrs, children) {
    ...
    let i = 0; const len = (children[0] && children.length) || 0
  ...
  function hsv2rgb (hsv) {
    ...
    return { r, g, b, hex: '#' + (16777216 | b | (g << 8) | (r << 16)).toString(16).slice(1) }
  }
  ...
  function slideListener (ctx, slideElement, pickerElement) {
    return function (evt) {
      ...
      ctx.s = ctx.v = 1
      const c = hsv2rgb({ h: ctx.h, s: 1, v: 1 })
  ...
  function ColorPicker (slideElement, pickerElement, callback) {
    ...
    if (!callback) {
      ...
      const slideIndicator = element.getElementsByClassName('slide-indicator')[0]
      const pickerIndicator = element.getElementsByClassName('picker-indicator')[0]
    ...
    if (type == 'SVG') {
      ...
      const hsvGradient = slide.getElementsByTagName('linearGradient')[0]
      const hsvRect = slide.getElementsByTagName('rect')[0]

      hsvGradient.id = 'gradient-hsv-' + uniqID
      hsvRect.setAttribute('fill', 'url(#' + hsvGradient.id + ')')

      const blackAndWhiteGradients = picker.getElementsByTagName('linearGradient')
      const whiteAndBlackRects = picker.getElementsByTagName('rect')
