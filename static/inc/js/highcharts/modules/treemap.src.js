...
(function (H) {
  ...
  const extendClass = H.extendClass
  const defaultOptions = H.getOptions()
  const plotOptions = defaultOptions.plotOptions
  const noop = function () { }
  const each = H.each
  const grep = HighchartsAdapter.grep
  const pick = H.pick
  const Series = H.Series
  const Color = H.Color
  const eachObject = function (list, func, context) {
    let key
  ...
  const reduce = function (arr, func, previous, context) {
    context = context || this
  ...
  var recursive = function (item, func, context) {
    let next
    context = context || this
  ...
  seriesTypes.treemap = extendClass(seriesTypes.scatter, merge(colorSeriesMixin, {
    ...
    /**
		 ...
		 */
    getListOfParents (data, ids) {
      const listOfParents = reduce(data, function (prev, curr, i) {
    ...
    getTree () {
      let tree
      const series = this
      const allIds = map(this.data, function (d) {
        return d.id
      })
      const parentList = series.getListOfParents(this.data, allIds)
    ...
    buildNode (id, i, level, list, parent) {
      const series = this
      const children = []
    ...
    setTreeValues (tree) {
      ...
      const sorted = []
      let val
      const point = series.points[tree.i]
    ...
    calculateChildrenAreas (parent, area) {
      const series = this
      const options = series.options
      const level = this.levelMap[parent.levelDynamic + 1]
      const algorithm = pick((series[level && level.layoutAlgorithm] && level.layoutAlgorithm), options.layoutAlgorithm)
      const alternate = options.alternateStartingDirection
      let childrenValues = []
      let children
    ...
    alg_func_calcPoints (directionChange, last, group, childrenArea) {
      let pX
      let pY
      let pW
      let pH
    ...
    alg_func_lowAspectRatio (directionChange, parent, children) {
      const childrenArea = []
      const series = this
      let pTot
      const plot = {
      ...
      const group = new this.alg_func_group(parent.height, parent.width, direction, plot)
    ...
    alg_func_fill (directionChange, parent, children) {
      const childrenArea = []
      let pTot
    ...
    drawDataLabels () {
      const series = this
      const points = grep(series.points, function (n) {
    ...
    pointAttribs (point, state) {
      const level = this.levelMap[point.node.levelDynamic] || {}
      const options = this.options
      let attr
      const stateOptions = (state && options.states[state]) || {}
    ...
    drawPoints () {
      ...
      each(points, function (point) {
        const groupKey = 'levelGroup-' + point.node.levelDynamic
        if (!series[groupKey]) {
    ...
    /**
		 ...
		 */
    insertElementSorted (arr, el, cond) {
      let i = 0
      let inserted = false
      if (arr.length !== 0) {
    ...
    showDrillUpButton (name) {
      const series = this
      let backText = (name || '< Back')
      const buttonOptions = series.options.drillUpButton
      let attr
      let states
