...
(function (Highcharts) {
  ...
  const Legend = Highcharts.Legend
 ...
 */
  const ColorAxis = Highcharts.ColorAxis = function () {
  ...
  extend(ColorAxis.prototype, {
    ...
    init (chart, userOptions) {
      const horiz = chart.options.legend.layout !== 'vertical'
      let options
    ...
    initDataClasses (userOptions) {
      ...
      const len = userOptions.dataClasses.length
    ...
    setAxisSize () {
      const symbol = this.legendSymbol
      const chart = this.chart
      let x
      let y
      let width
    ...
    toColor (value, point) {
      ...
      let to
      let color
      const dataClasses = this.dataClasses
      let dataClass
      let i
    ...
    getOffset () {
      ...
      const sideOffset = this.chart.axisOffset[this.side]
    ...
    drawLegendSymbol (legend, item) {
      ...
      const horiz = this.horiz
      let box
      const width = pick(legendOptions.symbolWidth, horiz ? 200 : 12)
      const height = pick(legendOptions.symbolHeight, horiz ? 12 : 200)
      const labelPadding = pick(legendOptions.labelPadding, horiz ? 16 : 30)
    ...
    drawCrosshair (e, point) {
      ...
      let crossPos
      const axisPos = this.pos
      const axisLen = this.len
    ...
    getDataClassLegendSymbols () {
      ...
      const valueDecimals = legendOptions.valueDecimals
      const valueSuffix = legendOptions.valueSuffix || ''
      let name
 ...
 */
  wrap(Chart.prototype, 'getAxes', function (proceed) {
    const options = this.options
    const colorAxisOptions = options.colorAxis
 ...
 */
  wrap(Legend.prototype, 'getAllItems', function (proceed) {
    let allItems = []
    const colorAxis = this.chart.colorAxis[0]
 ...
 */
  ...
  const colorSeriesMixin = {
    ...
    translateColors () {
      const series = this
      const nullColor = this.options.nullColor
  ...
  seriesTypes.heatmap = extendClass(seriesTypes.scatter, merge(colorSeriesMixin, {
    ...
    translate () {
      ...
      each(series.points, function (point) {
        ...
        const x1 = between(Math.round(xAxis.len - xAxis.translate(point.x - xPad, 0, 1, 0, 1)), 0, xAxis.len)
        const x2 = between(Math.round(xAxis.len - xAxis.translate(point.x + xPad, 0, 1, 0, 1)), 0, xAxis.len)
        const y1 = between(Math.round(yAxis.translate(point.y - yPad, 0, 1, 0, 1)), 0, yAxis.len)
        const y2 = between(Math.round(yAxis.translate(point.y + yPad, 0, 1, 0, 1)), 0, yAxis.len)
