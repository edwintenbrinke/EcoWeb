...
*/
function RGBColor (m) {
  this.ok = !1; m.charAt(0) == '#' && (m = m.substr(1, 6)); var m = m.replace(/ /g, ''); var m = m.toLowerCase(); const a = {
    aliceblue: 'f0f8ff',
    antiquewhite: 'faebd7',
    aqua: '00ffff',
    aquamarine: '7fffd4',
    ...
    yellow: 'ffff00',
    yellowgreen: '9acd32'
  }; let c; for (c in a) { m == c && (m = a[c]) } const d = [{ re: /^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/, example: ['rgb(123, 234, 45)', 'rgb(255,234,245)'], process (b) { return [parseInt(b[1]), parseInt(b[2]), parseInt(b[3])] } }, {
    re: /^(\w{2})(\w{2})(\w{2})$/,
    example: ['#00ff00', '336699'],
    process (b) { return [parseInt(b[1], 16), parseInt(b[2], 16), parseInt(b[3], 16)] }
  }, { re: /^(\w{1})(\w{1})(\w{1})$/, example: ['#fb0', 'f0f'], process (b) { return [parseInt(b[1] + b[1], 16), parseInt(b[2] + b[2], 16), parseInt(b[3] + b[3], 16)] } }]; for (c = 0; c < d.length; c++) { const b = d[c].process; const k = d[c].re.exec(m); if (k) { channels = b(k), this.r = channels[0], this.g = channels[1], this.b = channels[2], this.ok = !0 } } this.r = this.r < 0 || isNaN(this.r) ? 0 : this.r > 255 ? 255 : this.r; this.g = this.g < 0 || isNaN(this.g) ? 0
  ...
  function m () {
    const a = { FRAMERATE: 30, MAX_VIRTUAL_PIXELS: 3e4 }; a.init = function (c) {
      a.Definitions = {}; a.Styles = {}; a.Animations = []; a.Images = []; a.ctx = c; a.ViewPort = new function () {
        ...
        this.ComputeSize = function (a) { return a != null && typeof a === 'number' ? a : a == 'x' ? this.width() : a == 'y' ? this.height() : Math.sqrt(this.width() ** 2 + this.height() ** 2) / Math.sqrt(2) }
      }()
    }; a.init(); a.ImagesLoaded = function () { for (let c = 0; c < a.Images.length; c++) { if (!a.Images[c].loaded) { return !1 } } return !0 }; a.trim = function (a) { return a.replace(/^\s+|\s+$/g, '') }; a.compressSpaces = function (a) { return a ? a.replace(/[\s\r\t\n]+/gm, ' ') : '' }; a.ajax = function (a) {
...
(b /= 100); return b
      }; this.valueOrDefault = function (b) { return this.hasValue() ? this.value : b }; this.numValueOrDefault = function (b) { return this.hasValue() ? this.numValue() : b }; const b = this; this.Color = { addOpacity (d) { let c = b.value; if (d != null && d != '') { const f = new RGBColor(b.value); f.ok && (c = 'rgba(' + f.r + ', ' + f.g + ', ' + f.b + ', ' + d + ')') } return new a.Property(b.name, c) } }; this.Definition = {
        getDefinition () { const d = b.value.replace(/^(url\()?#([^\)]+)\)?$/, '$2'); return a.Definitions[d] },
        isUrl () {
          return b.value.indexOf('url(') ==
0
        },
        getFillStyle (b) { const d = this.getDefinition(); return d != null && d.createGradient ? d.createGradient(a.ctx, b) : d != null && d.createPattern ? d.createPattern(a.ctx, b) : null }
      }; this.Length = {
        DPI () { return 96 },
        EM (b) { let d = 12; const c = new a.Property('fontSize', a.Font.Parse(a.ctx.font).fontSize); c.hasValue() && (d = c.Length.toPixels(b)); return d },
        toPixels (d) {
      ...
      }; this.Time = { toMilliseconds () { if (!b.hasValue()) { return 0 } const a = b.value + ''; if (a.match(/s$/)) { return b.numValue() * 1e3 } a.match(/ms$/); return b.numValue() } }; this.Angle = {
        toRadians () {
...
g.fontFamily,
        ...
        for (var b = {}, d = a.trim(a.compressSpaces(d || '')).split(' '), k = !1, e = !1, f = !1, g = !1, j = '', h = 0; h < d.length; h++) {
          if (!e && c.Styles.includes(d[h])) { if (d[h] != 'inherit') { b.fontStyle = d[h] }e = !0 } else if (!g && c.Variants.includes(d[h])) {
            if (d[h] != 'inherit') {
              b.fontVariant =
d[h]
            }e = g = !0
          } else if (!f && c.Weights.includes(d[h])) { if (d[h] != 'inherit') { b.fontWeight = d[h] }e = g = f = !0 } else if (k) { d[h] != 'inherit' && (j += d[h]) } else { if (d[h] != 'inherit') { b.fontSize = d[h].split('/')[0] }e = g = f = k = !0 }
...
a); this.addBezierCurve(b, a, d, d + 1 / 3 * (k - b), c, c + 1 / 3 * (l - a), k, l)
        ...
        const q = [b, a]; const p = [d, c]; const t = [k, l]; const m = [o, n]; this.addPoint(q[0], q[1]); this.addPoint(m[0], m[1]); for (i = 0; i <= 1; i++) {
          b = function (b) { return (1 - b) ** 3 * q[i] + 3 * (1 - b) ** 2 * b * p[i] + 3 * (1 - b) * b ** 2 * t[i] + b ** 3 * m[i] }, a = 6 * q[i] - 12 * p[i] + 6 * t[i], d = -3 * q[i] + 9 * p[i] - 9 * t[i] + 3 * m[i], c = 3 * p[i] - 3 * q[i], d == 0 ? a != 0 && (a = -c / a, a > 0 && a < 1 && (i == 0 && this.addX(b(a)), i == 1 && this.addY(b(a)))) : (c = a ** 2 - 4 * c * d, c < 0 || (k =
...
b.current; var f = b.getPoint(); var j = b.getAsControlPoint(); var e = b.getAsCurrentPoint(); b.addMarker(e, j, f); c.addBezierCurve(g.x, g.y, f.x, f.y, j.x, j.y, e.x, e.y); d != null && d.bezierCurveTo(f.x, f.y, j.x, j.y, e.x, e.y)
              ...
              var e = b.getAsCurrentPoint(); var n = new a.Point(Math.cos(f) * (g.x - e.x) / 2 + Math.sin(f) * (g.y - e.y) / 2, -Math.sin(f) * (g.x - e.x) / 2 + Math.cos(f) * (g.y - e.y) / 2); var q = (n.x) ** 2 / h ** 2 + (n.y) ** 2 / l ** 2; q > 1 && (h *= Math.sqrt(q), l *= Math.sqrt(q)); o = (o == j ? -1 : 1) * Math.sqrt((h ** 2 * l ** 2 - h ** 2 * (n.y) ** 2 - l ** 2 * (n.x) ** 2) / (h ** 2 * (n.y) ** 2 + l ** 2 * (n.x) ** 2)); isNaN(o) && (o = 0); var p = new a.Point(o * h * n.y / l, o * -l * n.x / h); var g = new a.Point((g.x + e.x) / 2 + Math.cos(f) *
p.x - Math.sin(f) * p.y, (g.y + e.y) / 2 + Math.sin(f) * p.x + Math.cos(f) * p.y); var m = function (b, a) { return (b[0] * a[0] + b[1] * a[1]) / (Math.sqrt((b[0]) ** 2 + (b[1]) ** 2) * Math.sqrt((a[0]) ** 2 + (a[1]) ** 2)) }; const s = function (b, a) { return (b[0] * a[1] < b[1] * a[0] ? -1 : 1) * Math.acos(m(b, a)) }; var o = s([1, 0], [(n.x - p.x) / h, (n.y - p.y) / l]); var q = [(n.x - p.x) / h, (n.y - p.y) / l]; var p = [(-n.x - p.x) / h, (-n.y - p.y) / l]; var n = s(q, p); if (m(q, p) <= -1) { n = Math.PI }m(q, p) >= 1 && (n = 0); j == 0 && n > 0 && (n -= 2 * Math.PI); j == 1 && n < 0 && (n += 2 * Math.PI); q = new a.Point(g.x - h * Math.cos((o + n) /
...
function (b) {
  for (let a = this.style('text-anchor').valueOrDefault('start'), c = this.attribute('x').Length.toPixels('x'), d = this.attribute('y').Length.toPixels('y'), j = 0; j < this.children.length; j++) {
    const h = this.children[j]; h.attribute('x').hasValue() ? h.x = h.attribute('x').Length.toPixels('x') : (h.attribute('dx').hasValue() && (c += h.attribute('dx').Length.toPixels('x')), h.x = c); c = h.measureText ? h.measureText(b) : 0; if (a != 'start' && (j == 0 || h.attribute('x').hasValue())) {
...
'isolated'; if ((c == 0 || b[c - 1] == ' ') && c < b.length - 2 && b[c + 1] != ' ') { g = 'terminal' }c > 0 && b[c - 1] != ' ' && c < b.length - 2 && b[c + 1] != ' ' && (g = 'medial'); if (c > 0 && b[c - 1] != ' ' && (c == b.length - 1 || b[c + 1] == ' ')) { g = 'initial' } typeof a.glyphs[e] !== 'undefined' && (f = a.glyphs[e][g], f == null && a.glyphs[e].type == 'glyph' && (f = a.glyphs[e]))
        ...
        const b = this.parent.style('font-family').Definition.getDefinition(); if (b != null) {
          ...
          const e = this.parent.style('font-style').valueOrDefault(a.Font.Parse(a.ctx.font).fontStyle); let f = this.getText(); b.isRTL && (f = f.split('').reverse().join('')); for (let g = a.ToNumberArray(this.parent.attribute('dx').value), j = 0; j < f.length; j++) {
...
l, -1 / l); c.translate(-this.x, -this.y); this.x += k * (h.horizAdvX || b.horizAdvX) / b.fontFace.unitsPerEm; typeof g[j] !== 'undefined' && !isNaN(g[j]) && (this.x += g[j])
        ...
        let b = this.parent.style('font-family').Definition.getDefinition(); if (b != null) {
          ...
          let k = 0; let e = this.getText(); b.isRTL && (e = e.split('').reverse().join('')); for (let f = a.ToNumberArray(this.parent.attribute('dx').value), g = 0; g < e.length; g++) { const j = this.getGlyph(b, e, g); k += (j.horizAdvX || b.horizAdvX) * c / b.fontFace.unitsPerEm; typeof f[g] !== 'undefined' && !isNaN(f[g]) && (k += f[g]) } return k
...
0
      }; this.onclick = function (a, d) { this.events.push({ type: 'onclick', x: a, y: d, run (a) { if (a.onclick) { a.onclick() } } }) }; this.onmousemove = function (a, d) { this.events.push({ type: 'onmousemove', x: a, y: d, run (a) { if (a.onmousemove) { a.onmousemove() } } }) }; this.eventElements = []; this.checkPath = function (a, d) { for (let b = 0; b < this.events.length; b++) { const k = this.events[b]; d.isPointInPath && d.isPointInPath(k.x, k.y) && (this.eventElements[b] = a) } }; this.checkBoundingBox = function (a, d) {
...
(function (m) {
  const a = m.css; const c = m.CanVGRenderer; const d = m.SVGRenderer; const b = m.extend; const k = m.merge; const e = m.addEvent; const f = m.createElement; const g = m.discardElement; b(c.prototype, d.prototype); b(c.prototype, {
    create (a, b, c, d) { this.setContainer(b, c, d); this.configure(a) },
    setContainer (a, b, c) {
    ...
    configure (b) {
      const c = this; const d = b.options.tooltip; const f = d.borderWidth; const g = c.ttDiv; var m = d.style; const p = c.ttLine; const t = parseInt(m.padding, 10); var m = k(m, { padding: t + 'px', 'background-color': d.backgroundColor, 'border-style': 'solid', 'border-width': f + 'px', 'border-radius': d.borderRadius + 'px' }); d.shadow && (m = k(m, {
    ...
    destroy () { g(this.canvas); this.ttTimer !== void 0 && clearTimeout(this.ttTimer); g(this.ttLine); g(this.ttDiv); g(this.hiddenSvg); return d.prototype.destroy.apply(this) },
    color (a, b, c) { a && a.linearGradient && (a = a.stops[a.stops.length - 1][1]); return d.prototype.color.call(this, a, b, c) },
    draw () { window.canvg(this.canvas, this.hiddenSvg.innerHTML) }
  })
})(Highcharts)
