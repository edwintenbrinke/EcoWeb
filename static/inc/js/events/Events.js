(function () {
  ...
  Events.prototype.appendTitle = function (title, id) {
    const html = '<div class="row"> <h3 class="events-group-' + id + '">' + title + '<div class="class-discution-' + id + ' class-discussion"></div></h3> <div class="col-md-12 event-wrapper-2"> <div class="map-' + id + '"></div> <div class="graph-' + id + '"></div><div class="chat-snippets-' + id + '"></div></div> </div> </div>'
  ...
  Events.prototype.appendChatSnippets = function (data, id) {
    ...
    for (const k in data) {
      for (const i in data[k]) {
        const text = this.StringFiltr(data[k][i].Text)

        let html = '<p><span class="curr-event-chat-tag">' + data[k][i].Tag + '</span> <span class="curr-event-chat-user">' + data[k][i].Username + ':</span> <span class="curr-event-chat-text">' + text + '</span></p>'
        ...
        if (data[k][i].Map) {
          const settings = JSON.stringify(data[k][i].Map)
  ...
  Events.prototype.StringFiltr = function (text) {
    ...
    function x (t) {
      ...
      if (c >= 0) {
        ...
        let v = t.substring(c, t.length)
        const b = v.substring(0, 3)
        const b1 = v.substring(0, 4)
  ...
  Events.prototype.appendClassDiscution = function (className, id) {
    const place = $('.class-discution-' + id)
  ...
  Events.prototype.eventBackground = function () {
    ...
    function checkForChanges () {
      const	offset = $('.events-table').offset()
      const element1 = $('.events-table')
      const element2 = $('.curr-event-background')
