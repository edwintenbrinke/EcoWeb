;(function () {
  ...
  Variables.prototype.showError = function (errorResponse) {
    const message = errorResponse.responseJSON && errorResponse.responseJSON.Message || errorResponse.responseText || errorResponse.statusText || errorResponse
  ...
  Variables.prototype.setDistrict = function (func) {
    ...
    for (let i = 0; i < array.length; i++) {
      const item = array[i]
      const color = $(item).css('background-color').split(')')[0].split('(')[1].split(', ')
      const district = {
  ...
  Variables.prototype.showServer = function (obj) {
    return new Promise(function (resolve, reject) {
      const data = obj.this.Info
      const description_arr = data.Description.split(' ')
      let description = ''

      for (i in description_arr) {
        const text = description_arr[i]
  ...
  Variables.prototype.Dashboard = function (obj) {
    return new Promise(function (resolve, reject) {
      if (currentPage == 'dashboard') {
        const data = obj.this.Info
        const row = "<b>Server Version: </b><span class='text-highlight'>" + data.Version + '</span>'
  ...
  Variables.prototype.SavedDiscussionsMenu = function (obj) {
    const data = obj.this.SavedDiscussions
  ...
  Variables.prototype.RunForOffice = function (obj) {
    const data = obj.this.currentElections
    return new Promise(function (resolve, reject) {
  ...
  Variables.prototype.addSelfCandidate = function (obj) {
    const Speech = variables.Speech
    $('.error').empty().fadeOut()

    const content = $('.fancy-add-speech')
  ...
  Variables.prototype.addSpeech = function (obj) {
    const speech = encodeURIComponent($('#speech').val())
  ...
  Variables.prototype.RunningElectionsMenu = function (obj) {
    const data = obj.this.runningElections
  ...
  Variables.prototype.PreviousElectionsMenu = function (obj) {
    const data = obj.this.previousElections
