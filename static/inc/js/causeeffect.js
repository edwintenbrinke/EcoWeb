;(function () {
  ...
  CauseEffect.prototype.resolveData = function (obj) {
    return new Promise(function (resolve, reject) {
      ...
      const nodes = obj.this.graph.nodes
      const conns = obj.this.graph.conns
      ...
      const maxHorCount = Math.max(horCount[0], horCount[1], horCount[2])
      hor = 4000 / (3 * maxHorCount)

      _this.nodeSize.size = Math.max(_this.nodeSize.min, Math.min(_this.nodeSize.max, ver, hor))

      const maxWidth = _this.nodeSize.size * (maxHorCount + 1) * 5 / 4

      const width = Math.max(maxWidth, 2000)
      const height = (maxHorCount > 7 && verCount == 3) ? 2000 : verCount == 1 ? 700 : 1200
      ...
      const realHeight = parseInt($('#causeeffectcontainer').css('height')) - 100
      const realWidth = realHeight * width / 1000
  ...
  CauseEffect.prototype.initSVG = function (obj) {
    ...
    const svg = d3.select('svg#causeeffectsvg')
    const svg1 = d3.select('svg#causeeffectsvg1')
    const focused = _this.focused
    ...
    const realWidth = parseInt($('#causeeffectsvg').css('width'))
    const realHeight = parseInt($('#causeeffectsvg').css('height'))
    const kx = _this.width / realWidth
    const ky = _this.height / realHeight
  ...
  CauseEffect.prototype.appendMap = function (node, stat) {
    ...
    if (stat == false) {
      ...
      ECO.MINIMAP.addView(view)
      const borderSize = 0
      const area = ECO.MINIMAP.getWorldArea(view, borderSize)
  ...
  CauseEffect.prototype.deleteMap = function (LayerName) {
    ...
    const view = $('.map-' + LayerName + ' .view-map')
  ...
  CauseEffect.prototype.getCoords = function (d, r) {
    ...
    const	w = causeeffect.nodeSize.size + 10
    const h = causeeffect.nodeSize.size + 80
  ...
  CauseEffect.prototype.updateMapsPos = function (d) {
    const _this = causeeffect
    if (d.class != 'node' || $('#causeeffectsvg').length == 0) { return }
    const i = d.LayerName
    const realWidth = $('#causeeffectsvg').width()
    const realHeight = $('#causeeffectsvg').height()

    const viewBox = $('#causeeffectsvg')[0].attributes[_this.attrIndex].value.split(' ')

    const factionWidth = viewBox[2] - viewBox[0]
    const factionHeight = viewBox[3] - viewBox[1]

    const kx = realWidth / factionWidth
    const ky = realHeight / factionHeight

    const width = _this.nodeSize.size * kx
    const height = _this.nodeSize.size * ky
  ...
  CauseEffect.prototype.changeTimeRange = function (t, item) {
    ...
    const max = parseFloat($(this).slider('option', 'max'))
  ...
  CauseEffect.prototype.slideTimeRange = function (e, item) {
    e = e || false
    item = item || false
    const value = (item && !$.isEmptyObject(item)) ? item.value : $('#slider').slider('option', 'value')
  ...
  CauseEffect.prototype.updateMapsInfo = function () {
    const _this = this
    const data = new Object()
    const area = ECO.MINIMAP.getWorldArea($('#map_' + _this.focused + ' .view-map')[0], 0)
  ...
  CauseEffect.prototype.textwidth = function (d, state) {
    ...
    if (state == 'nodeName') {
      ...
      const viewBox = $('#causeeffectsvg')[0].attributes[_this.attrIndex].value.split(' ')
      const factionWidth = viewBox[2] - viewBox[0]
      const kx = factionWidth / realWidth
  ...
  CauseEffect.prototype.bindEvent = function () {
    ...
    $(document).on('click', '#causeeffectcontainer .item span', function () {
      const graph_part = $(this).parent().find('.graph_part')

      $('.graph_part.active').slideUp().removeClass('active')

      if ($(graph_part).css('display') == 'block') { $(graph_part).slideUp() } else { $(graph_part).addClass('active').css({ 'margin-top': '-50px' }).slideDown() }
    })
    $(document).on('click', '#timeRange .range-control', function () {
      ...
      if ($(elem).hasClass('fa-play')) {
        ...
        _this.timeRange = setInterval(function () {
          const step = parseFloat($('#range-speed').slider('option', 'value')).toFixed(2)
          let time = parseFloat($('#slider').slider('value')) + step * 8640
          const max = parseFloat($('#slider').slider('option', 'max'))
          if (time >= max) { time = 0 }
    ...
    $(document).on('change', '#categoryselect', function () {
      ...
      const i = $(this).val()
      const d = _this.layers[i]
      _this.focusedcat = d.Category
      for (const j in d.List) {
        const layer = d.List[j]
      ...
      const w = $('#categoryselect').width() + $('#layerselect').width() + 670
    ...
    $(document).on('mouseover', '.connection, .text_part, .icon1', function (evt) {
      ...
      const factionHeight = viewBox[3] - viewBox[1]
      const kx = factionWidth / realWidth
      const ky = factionHeight / realHeight
      ...
      if (evt.target.nodeName == 'line' || evt.target.nodeName == 'rect') {
        const connection = $(this).attr('data-connection').replace(/[ -<>,()]/g, '')
      ...
      const coords = $('#causeeffectsvg1')[0].getBoundingClientRect()
      const x = (270 - (coords.x || coords.left)) * kx
      const y = (187 - (coords.y || coords.top)) * ky
    ...
    $(document).on('mouseout', '.connection, .text_part, .icon1', function (evt) {
      _this.tooltipHide = true
      const elem = this
      const LayerName = $(elem).attr('data-layername')

      setTimeout(function () {
        if (_this.tooltipHide) {
          if (evt.target.nodeName == 'line' || evt.target.nodeName == 'rect') {
            const connection = $(elem).attr('data-connection').replace(/[ -<>,()]/g, '')
