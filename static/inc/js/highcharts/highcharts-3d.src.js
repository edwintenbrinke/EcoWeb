 ...
 */
  function perspective (points, chart, insidePlotArea) {
    ...
    const ze = origin.z
    const vd = origin.vd
    const angle1 = deg2rad * (inverted ? options3d.beta : -options3d.beta)
    const angle2 = deg2rad * (inverted ? -options3d.alpha : options3d.alpha)
    const s1 = sin(angle1)
    const c1 = cos(angle1)
    const s2 = sin(angle2)
    const c2 = cos(angle2)
  ...
  */
  ...
  Highcharts.SVGRenderer.prototype.cuboid = function (shapeArgs) {
    const result = this.g()
    const paths = this.cuboidPath(shapeArgs)
    ...
    result.fillSetter = function (color) {
      const c0 = color
      const c1 = Highcharts.Color(color).brighten(0.1).get()
      const c2 = Highcharts.Color(color).brighten(-0.1).get()
    ...
    result.opacitySetter = function (opacity) {
      this.front.attr({ opacity })
      this.top.attr({ opacity })
      this.side.attr({ opacity })
      return this
    }

    result.attr = function (args) {
      if (args.shapeArgs || defined(args.x)) {
        const shapeArgs = args.shapeArgs || args
        const paths = this.renderer.cuboidPath(shapeArgs)
    ...
    result.animate = function (args, duration, complete) {
      if (defined(args.x) && defined(args.y)) {
        const paths = this.renderer.cuboidPath(args)
 ...
 */
  Highcharts.SVGRenderer.prototype.cuboidPath = function (shapeArgs) {
    ...
    const d = shapeArgs.depth
    const chart = Highcharts.charts[this.chartIndex]
    ...
    const pickShape = function (path1, path2) {
    ...
    const front = [3, 2, 1, 0]
    const back = [7, 6, 5, 4]
    const path1 = pickShape(front, back)

    // top or bottom
    const top = [1, 6, 7, 0]
    const bottom = [4, 5, 2, 3]
    const path2 = pickShape(top, bottom)

    // side
    const right = [1, 2, 5, 6]
    const left = [0, 7, 4, 3]
    const path3 = pickShape(right, left)
  ...
  Highcharts.SVGRenderer.prototype.arc3d = function (shapeArgs) {
    ...
    const result = this.g()
    const paths = this.arc3dPath(shapeArgs)
    const renderer = result.renderer

    const zIndex = paths.zTop * 100
      ...
      if (defined(args.end) || defined(args.start)) {
        ...
        Highcharts.SVGElement.prototype.animate.call(this, {
          ...
          step () {
            ...
            const start = result._shapeArgs
            const end = fx.end
            const pos = fx.pos
            const sA = Highcharts.merge(start, {
            ...
            const paths = result.renderer.arc3dPath(sA)
 ...
 */
  Highcharts.SVGRenderer.prototype.arc3dPath = function (shapeArgs) {
    const cx = shapeArgs.x // x coordinate of the center
    const cy = shapeArgs.y // y coordinate of the center
    const start = shapeArgs.start // start angle
    const end = shapeArgs.end - 0.00001 // end angle
    const r = shapeArgs.r // radius
    const ir = shapeArgs.innerR // inner radius
    const d = shapeArgs.depth // depth
    const alpha = shapeArgs.alpha // alpha rotation of the chart
    const beta = shapeArgs.beta // beta rotation of the chart

    // Derived Variables
    const cs = cos(start)		// cosinus of the start angle
    const ss = sin(start)		// sinus of the start angle
    const ce = cos(end)			// cosinus of the end angle
    const se = sin(end)			// sinus of the end angle
    const rx = r * cos(beta)		// x-radius
    const ry = r * cos(alpha)	// y-radius
    const irx = ir * cos(beta)	// x-radius (inner)
    const iry = ir * cos(alpha)	// y-radius (inner)
    const dx = d * sin(beta)		// distance between top and bottom in x
    const dy = d * sin(alpha)	// distance between top and bottom in y
    ...
    // OUTSIDE
    const b = (beta > 0 ? PI / 2 : 0)
    const a = (alpha > 0 ? 0 : PI / 2)
...
***/
  ...
  const defaultChartOptions = Highcharts.getOptions()
  ...
  Highcharts.wrap(Highcharts.Chart.prototype, 'setChartSize', function (proceed) {
    ...
    if (this.is3d()) {
      ...
      const x = inverted ? 'y' : 'x'
      const y = inverted ? 'x' : 'y'
      const w = inverted ? 'height' : 'width'
      const h = inverted ? 'width' : 'height'
...
***/
  ...
  Highcharts.wrap(Highcharts.Axis.prototype, 'render', function (proceed) {
    ...
    const options3d = chart.options.chart.options3d
    const frame = options3d.frame
    const fbottom = frame.bottom
    const fback = frame.back
    const fside = frame.side
    const depth = options3d.depth
  ...
  Highcharts.wrap(Highcharts.Axis.prototype, 'getPlotLinePath', function (proceed) {
    let path = proceed.apply(this, [].slice.call(arguments, 1))
    ...
    const d = this.isZAxis ? this.chart.plotWidth : options3d.depth
...
***/
  ...
  Highcharts.wrap(Highcharts.Tick.prototype, 'getLabelPosition', function (proceed) {
    ...
    const new_pos = perspective([this.axis.swapZ({ x: pos.x, y: pos.y, z: 0 })], this.axis.chart, false)[0]
  ...
  Highcharts.wrap(Highcharts.Axis.prototype, 'getTitlePosition', function (proceed) {
    let pos = proceed.apply(this, [].slice.call(arguments, 1))
...
***/
  ...
  Highcharts.Axis.prototype.swapZ = function (p, insidePlotArea) {
    if (this.isZAxis) {
      const plotLeft = insidePlotArea ? 0 : this.chart.plotLeft
      const chart = this.chart
      return {
  ...
  const ZAxis = Highcharts.ZAxis = function () {
    this.isZAxis = true
...
*/
  Highcharts.wrap(Highcharts.Chart.prototype, 'getAxes', function (proceed) {
    ...
    const zAxisOptions = options.zAxis = Highcharts.splat(options.zAxis || {})
    ...
    Highcharts.each(zAxisOptions, function (axisOptions, i) {
      ...
      const zAxis = new ZAxis(chart, axisOptions)
      zAxis.setScale()
    })
  })
  /***
...
***/
  Highcharts.wrap(Highcharts.seriesTypes.column.prototype, 'translate', function (proceed) {
    ...
    const seriesOptions = series.options
    const depth = seriesOptions.depth || 25

    const stack = seriesOptions.stacking ? (seriesOptions.stack || 0) : series._i
    let z = stack * (depth + (seriesOptions.groupZPadding || 1))
    ...
    Highcharts.each(series.data, function (point) {
      if (point.y !== null) {
        ...
        let tooltipPos = point.tooltipPos
        ...
        tooltipPos = perspective([{ x: tooltipPos[0], y: tooltipPos[1], z }], chart, false)[0]
  ...
  Highcharts.wrap(Highcharts.seriesTypes.column.prototype, 'animate', function (proceed) {
    if (!this.chart.is3d()) {
      ...
      const series = this
      const reversed = this.yAxis.reversed
  ...
  Highcharts.wrap(Highcharts.seriesTypes.column.prototype, 'init', function (proceed) {
    ...
    if (this.chart.is3d()) {
      ...
      const grouping = seriesOptions.grouping
      const stacking = seriesOptions.stacking
      let z = 0

      if (!(grouping !== undefined && !grouping)) {
        const stacks = this.chart.retrieveStacks(stacking)
        const stack = seriesOptions.stack || 0
  ...
  function draw3DPoints (proceed) {
    // Do not do this if the chart is not 3D
    if (this.chart.is3d()) {
      const grouping = this.chart.options.plotOptions.column.grouping
  ...
  Highcharts.wrap(Highcharts.Series.prototype, 'alignDataLabel', function (proceed) {
    ...
    if (this.chart.is3d() && (this.type === 'column' || this.type === 'columnrange')) {
      ...
      const args = arguments
      const alignTo = args[4]

      let pos = ({ x: alignTo.x, y: alignTo.y, z: series.z })
  ...
  Highcharts.wrap(Highcharts.seriesTypes.pie.prototype, 'drawDataLabels', function (proceed) {
    if (this.chart.is3d()) {
      ...
      Highcharts.each(series.data, function (point) {
        ...
        const r = shapeArgs.r
        const d = shapeArgs.depth
        const a1 = (shapeArgs.alpha || series.chart.options.chart.options3d.alpha) * deg2rad // #3240 issue with datalabels for 0 and null values
        const a2 = (shapeArgs.start + shapeArgs.end) / 2
        const labelPos = point.labelPos
...
***/
  ...
  Highcharts.wrap(Highcharts.seriesTypes.scatter.prototype, 'translate', function (proceed) {
    ...
    const series = this
    const chart = series.chart
    const zAxis = Highcharts.pick(series.zAxis, chart.options.zAxis[0])
    const rawPoints = []
    let rawPoint
    let projectedPoints
    let projectedPoint
  ...
  Highcharts.wrap(Highcharts.seriesTypes.scatter.prototype, 'init', function (proceed, chart, options) {
    ...
    const result = proceed.apply(this, [chart, options])

    if (this.chart.is3d()) {
      // Set a new default tooltip formatter
      const default3dScatterTooltip = 'x: <b>{point.x}</b><br/>y: <b>{point.y}</b><br/>z: <b>{point.z}</b><br/>'
 ...
 */
  if (Highcharts.VMLRenderer) {
    ...
    Highcharts.VMLRenderer.prototype.arc3d = function (shapeArgs) {
      const result = Highcharts.SVGRenderer.prototype.arc3d.call(this, shapeArgs)
