...
function PreviousElectionsMenu (data) {
  if (data.length == 0) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const item = $("<li class='" + highlight + "'><a href='/elections.html?election=" + data[i].Id + "' class='previous-election' data-guid = '" + data[i].Id + "'><span>" + data[i].Name + '</span></li>')
...
function RunningElectionsMenu (data) {
  if (data.length === 0) {
    ...
    $('li#running-elections > a').html("<span class='localize' translate-key='164'>No Running Elections</span>")
    $('li#running-elections > a').addClass('menu-no-link')
    return false
  } else {
    // adding previous elections
    $('li#running-elections > a').html("<span class='localize' translate-key='163'>Running Elections</span><span class='caret'></span>")
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const item = $("<li class='" + highlight + "'><a href='/elections.html?election=" + data[i].Id + "' class='running-election' data-guid = '" + data[i].Id + "'><span>" + data[i].Name + '</span></a></li>')
