;(function () {
  ...
  Teacher.prototype.appendElection = function (obj) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (obj.activity.length) {
        const table = $('.teacher-row')
        const activity = obj.activity
        var data = obj.data
        const electionType = obj.electionType
        const summary = ''
  ...
  Teacher.prototype.electionHtml = function (data, time) {
    ...
    for (i in activity) {
      if (electionType == 'current' && election && election.Guid == activity[i].Election.Guid) {
        ...
        if (summary) {
          _this.appendHTML({ summary, result: 'Projected Results: ' + election.Results.Result, table, name: 'Election proposed by ' + activity[i].Election.Proposer, link: APPIQUERYURL + '/elections.html?election=current' })
        }
        ...
        for (k in election) {
          if (election[k].Guid == activity[i].Election.Guid) {
            summary = _this.joinEvents(activity[i].Events, time)
            if (summary) {
              _this.appendHTML({ summary, result: election[k].Results.Result, table, name: 'Election proposed by ' + activity[i].Election.Proposer, link: APPIQUERYURL + '/elections.html?election=' + election[k].Guid })
            }
          }
        }
      }
      ...
      for (k in lows) {
        if (lows[k].Guid == activity[i].Law.Guid) {
          ...
          if (summary) {
            ...
            _this.appendHTML({ summary, result, table, name: lows[k].Title, link: APPIQUERYURL + '/laws.html?lawid=' + lows[k].Guid })
          }
        }
      }
    }
  ...
  Teacher.prototype.joinEvents = function (events, time) {
    return events.filter(function (elem) {
      ...
      const players = elem.Players.join(', ')
