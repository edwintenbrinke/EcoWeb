;(function () {
  ...
  Discussion.prototype.saveDiscussion = function () {
    ...
    let k = 0
    for (const i in this.Discussion.Activities) {
  ...
  Discussion.prototype.discussionContent = function (obj) {
    return new Promise(function (resolve, reject) {
      const data = obj.this.Discussion
  ...
  Discussion.prototype.appendTitle = function (title, description) {
    let html = '<div class="section-wrapper">'
  ...
  Discussion.prototype.appendContent = function (data, id) {
    let htmlTitle = '<div class="activit" data-id="' + id + '">'
    let select = '<select data-pk="' + id + '" class="discussion-format" style="display: inline;"><option value="Data" selected>Data</option><option value="Discussion">Discussion</option><option value="Causes And Effects">Causes And Effects</option><option value="Reflection">Reflection</option></select>'
    let description = ''
    if (data.Format == 'Data') {
  ...
  Discussion.prototype.appendMap = function (obj, id, i) {
    ...
    const _this = this
    const place = $('.map-' + id)
    let html = '<div class="discussion-map map-' + id + '-' + i + '" data-id="' + id + '" data-j="' + i + '">'
    ...
    $('.map-' + id + '-' + i + ' div').load('../map.html', function (e) {
      ...
      const data = _this.Discussion.Activities
      const i1 = $(this).parent().data('id')
      const i2 = $(this).parent().data('j')
      const view = $('.map-' + id + '-' + i + ' .view-map')[0]
      let pause = true

      if (Object.keys(data[i1].Data.Maps[i2]).length) {
        view.settings = data[i1].Data.Maps[i2]
        view.settings.pause = false
        const layerSelected = data[i1].Data.Maps[i2].layerSelected
  ...
  Discussion.prototype.appendGraph = function (obj, id, i) {
    this.iframe_count++
    const _this = this
    const place = $('.graph-' + id)
    let html = '<div class=" discussionform-graph graph-' + id + '-' + i + '">'
      ...
      if (Object.keys(_this.Discussion.Activities[i]).length) {
        if (_this.Discussion.Activities[i].Data.Graphs.length) {
          for (j in _this.Discussion.Activities[i].Data.Graphs) {
            let currentGraphs = $('.graph-' + i + '-' + j + ' .graph-data').val()
            if (currentGraphs) {
        ...
        if (_this.Discussion.Activities[i].Data.Maps.length) {
          for (j in _this.Discussion.Activities[i].Data.Maps) {
            ...
            let currentMaps = $('.map-' + i + '-' + j + ' .map-data').val()
            if (currentMaps) {
  ...
  Discussion.prototype.enableEdit = function () {
    ...
    editable.on('shown', function (e, editable) {
      const textarea = $('.editable-container textarea')[0]
  ...
  Discussion.prototype.printDiscussionForm = function () {
    ...
    iframe.onload = function () {
      ...
      let html = '<table style="margin-top:50px; width: 100%;font-family: Dosis;">' +
        ...
        if (Object.keys(dataActivities[i]).length) {
          if (dataActivities[i].Data.Graphs.length) {
            for (j in dataActivities[i].Data.Graphs) {
              ...
              const Keys = JSON.stringify(dataActivities[i].Data.Graphs[j].Keys)
          ...
          if (dataActivities[i].Data.Maps.length) {
            for (j in dataActivities[i].Data.Maps) {
              ...
              const settings = JSON.stringify(dataActivities[i].Data.Maps[j])
              maps_count++
      ...
      const x = doc.getElementsByTagName('body')[0]
  ...
  Discussion.prototype.addGraphandMapEvent = function (id) {
    ...
    $(document).on('click', '.addGraph-' + id, function () {
      const i = _this.Discussion.Activities[id].Data.Graphs.length
      _this.appendGraph(false, id, i)
    })

    $(document).on('click', '.addMap-' + id, function () {
      const i = _this.Discussion.Activities[id].Data.Maps.length
